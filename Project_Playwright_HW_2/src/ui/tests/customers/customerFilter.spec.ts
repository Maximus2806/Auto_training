...
test.describe('[UI] [Customers] [Filter]', async () => {
    ...
    countries.forEach(country => {
        test(`Should filter customers by ${country}`, async function({customersPage, customersPageService}) {
            await customersPage.clickOnFilterButton();
            await customersPage.setFilter(country);
            await customersPage.applyFilter();
            await customersPage.waitForOpened();            
            await customersPageService.validateFilterResults("country", country)            
        })
    })
})